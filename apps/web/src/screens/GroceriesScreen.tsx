...
import groceriesData from "@baseline/content/okinawa/modules/groceries.json";
...
export default function GroceriesScreen() {
  ...
  const [activeId, setActiveId] = useState<string>(groceriesData[0].id);
  const active = groceriesData.find((g) => g.id === activeId) as GroceryCategory;
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: "var(--color-surface)" }}>
      ...
      <div className="px-5 pt-10 pb-4" style={{ backgroundColor: "var(--color-brand)" }}>
        ...
        <p className="text-blue-200 text-sm mt-1">Where to shop on and off base</p>
      </div>
      ...
      >
        {groceriesData.map((g) => (
          <button
            ...
            `}
            style={activeId === g.id ? { backgroundColor: "var(--color-brand)" } : {}}
          >
